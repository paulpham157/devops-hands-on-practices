...
These instructions apply to `09-k3s-local-cluster/` (Lesson 09: K3s Local Cluster with k3d). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 09-k3s-local-cluster -path '*/scripts/*.sh' -exec sh -n {} \;
```
