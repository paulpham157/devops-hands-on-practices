...
These instructions apply to `27-data-management/` (Lesson 27: Data Management). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 27-data-management -path '*/scripts/*.sh' -exec sh -n {} \;
cd 27-data-management && ./scripts/validate.sh
```
