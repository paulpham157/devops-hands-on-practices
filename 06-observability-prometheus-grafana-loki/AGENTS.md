...
These instructions apply to `06-observability-prometheus-grafana-loki/` (Lesson 06: Observability with Prometheus, Grafana, Loki, and Alloy). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 06-observability-prometheus-grafana-loki -path '*/scripts/*.sh' -exec sh -n {} \;
```
