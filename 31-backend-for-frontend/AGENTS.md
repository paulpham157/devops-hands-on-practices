...
These instructions apply to `31-backend-for-frontend/` (Lesson 31: Backend for Frontend). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 31-backend-for-frontend -path '*/scripts/*.sh' -exec sh -n {} \;
cd 31-backend-for-frontend && ./scripts/validate.sh
```
