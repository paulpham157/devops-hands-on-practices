...
These instructions apply to `32-sla-sli-slo/` (Lesson 32: SLA, SLI, SLO). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 32-sla-sli-slo -path '*/scripts/*.sh' -exec sh -n {} \;
cd 32-sla-sli-slo && ./scripts/validate.sh
```
