...
These instructions apply to `21-aws-azure-gcp-cloud-fundamentals/` (Lesson 21: AWS, Azure, and Google Cloud Platform Fundamentals). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
cd 21-aws-azure-gcp-cloud-fundamentals
./scripts/validate.sh
```
...
```bash
git diff --check
find 21-aws-azure-gcp-cloud-fundamentals -path '*/scripts/*.sh' -exec sh -n {} \;
cd 21-aws-azure-gcp-cloud-fundamentals && ./scripts/validate.sh
```
