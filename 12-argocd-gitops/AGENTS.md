...
These instructions apply to `12-argocd-gitops/` (Lesson 12: Argo CD GitOps on K3s). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 12-argocd-gitops -path '*/scripts/*.sh' -exec sh -n {} \;
```
