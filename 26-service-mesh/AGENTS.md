...
These instructions apply to `26-service-mesh/` (Lesson 26: Service Mesh). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 26-service-mesh -path '*/scripts/*.sh' -exec sh -n {} \;
cd 26-service-mesh && ./scripts/validate.sh
```
