...
These instructions apply to `33-snyk-and-sonar/` (Lesson 33: Snyk and Sonar). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 33-snyk-and-sonar -path '*/scripts/*.sh' -exec sh -n {} \;
cd 33-snyk-and-sonar && ./scripts/validate.sh
```
