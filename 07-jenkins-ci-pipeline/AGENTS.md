...
These instructions apply to `07-jenkins-ci-pipeline/` (Lesson 07: Jenkins CI Pipeline with Docker). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 07-jenkins-ci-pipeline -path '*/scripts/*.sh' -exec sh -n {} \;
```
