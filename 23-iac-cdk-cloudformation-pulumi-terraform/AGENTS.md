...
These instructions apply to `23-iac-cdk-cloudformation-pulumi-terraform/` (Lesson 23: AWS CDK, CloudFormation, Pulumi, and Terraform Extension Patterns). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
cd 23-iac-cdk-cloudformation-pulumi-terraform
./scripts/validate.sh
```
...
```bash
git diff --check
find 23-iac-cdk-cloudformation-pulumi-terraform -path '*/scripts/*.sh' -exec sh -n {} \;
cd 23-iac-cdk-cloudformation-pulumi-terraform && ./scripts/validate.sh
```
