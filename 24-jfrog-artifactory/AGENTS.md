...
These instructions apply to `24-jfrog-artifactory/` (Lesson 24: JFrog Artifactory). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 24-jfrog-artifactory -path '*/scripts/*.sh' -exec sh -n {} \;
cd 24-jfrog-artifactory && ./scripts/validate.sh
```
