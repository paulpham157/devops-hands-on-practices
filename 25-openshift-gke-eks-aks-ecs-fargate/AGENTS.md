...
These instructions apply to `25-openshift-gke-eks-aks-ecs-fargate/` (Lesson 25: OpenShift, GKE, EKS, AKS, ECS, and Fargate). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
cd 25-openshift-gke-eks-aks-ecs-fargate
./scripts/validate.sh
```
...
```bash
git diff --check
find 25-openshift-gke-eks-aks-ecs-fargate -path '*/scripts/*.sh' -exec sh -n {} \;
cd 25-openshift-gke-eks-aks-ecs-fargate && ./scripts/validate.sh
```
