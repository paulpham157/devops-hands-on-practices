...
These instructions apply to `28-availability-patterns/` (Lesson 28: Availability Patterns). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 28-availability-patterns -path '*/scripts/*.sh' -exec sh -n {} \;
cd 28-availability-patterns && ./scripts/validate.sh
```
