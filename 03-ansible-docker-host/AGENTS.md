...
These instructions apply to `03-ansible-docker-host/` (Lesson 03: Configure a Docker Host with Ansible). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 03-ansible-docker-host -path '*/scripts/*.sh' -exec sh -n {} \;
```
