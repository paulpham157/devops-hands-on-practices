...
These instructions apply to `36-chef-infra-automation/` (Lesson 36: Chef Infra Automation). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 36-chef-infra-automation -path '*/scripts/*.sh' -exec sh -n {} \
cd 36-chef-infra-automation && ./scripts/validate.sh
```
