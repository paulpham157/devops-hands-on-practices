...
These instructions apply to `20-application-protocols-microservices/` (Lesson 20: Application Protocols for Microservices). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 20-application-protocols-microservices -path '*/scripts/*.sh' -exec sh -n {} \;
cd 20-application-protocols-microservices && ./scripts/validate.sh
```
