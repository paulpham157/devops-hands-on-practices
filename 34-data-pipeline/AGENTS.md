...
These instructions apply to `34-data-pipeline/` (Lesson 34: Data Pipeline). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 34-data-pipeline -path '*/scripts/*.sh' -exec sh -n {} \;
cd 34-data-pipeline && ./scripts/validate.sh
```
