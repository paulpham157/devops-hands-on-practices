...
These instructions apply to `29-design-and-implementation-patterns/` (Lesson 29: Design and Implementation Patterns). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
cd 29-design-and-implementation-patterns
./scripts/validate.sh
```
...
```bash
git diff --check
find 29-design-and-implementation-patterns -path '*/scripts/*.sh' -exec sh -n {} \;
cd 29-design-and-implementation-patterns && ./scripts/validate.sh
```
