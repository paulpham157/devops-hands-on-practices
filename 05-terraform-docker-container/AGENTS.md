...
These instructions apply to `05-terraform-docker-container/` (Lesson 05: Manage a Docker Container with Terraform). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 05-terraform-docker-container -path '*/scripts/*.sh' -exec sh -n {} \;
```
