...
These instructions apply to `17-language-runtime-containerization/` (Lesson 17: Python, Java Spring, Ruby, Go, and Rust Containers). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
cd 17-language-runtime-containerization
./scripts/validate.sh
...
```bash
git diff --check
find 17-language-runtime-containerization -path '*/scripts/*.sh' -exec sh -n {} \;
cd 17-language-runtime-containerization && ./scripts/validate.sh
```
