...
These instructions apply to `04-packer-docker-image/` (Lesson 04: Build an Image with Packer). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 04-packer-docker-image -path '*/scripts/*.sh' -exec sh -n {} \;
```
