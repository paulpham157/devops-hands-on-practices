...
These instructions apply to `18-linux-unix-fundamentals/` (Lesson 18: Linux and Unix Fundamentals). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 18-linux-unix-fundamentals -path '*/scripts/*.sh' -exec sh -n {} \;
cd 18-linux-unix-fundamentals && ./scripts/validate.sh
```
