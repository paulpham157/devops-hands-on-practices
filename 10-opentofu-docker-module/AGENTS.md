...
These instructions apply to `10-opentofu-docker-module/` (Lesson 10: Manage Docker with OpenTofu Modules). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 10-opentofu-docker-module -path '*/scripts/*.sh' -exec sh -n {} \;
```
