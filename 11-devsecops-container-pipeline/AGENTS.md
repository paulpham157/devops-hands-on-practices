...
These instructions apply to `11-devsecops-container-pipeline/` (Lesson 11: DevSecOps for a Container Pipeline). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 11-devsecops-container-pipeline -path '*/scripts/*.sh' -exec sh -n {} \;
```
