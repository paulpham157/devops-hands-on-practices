...
These instructions apply to `35-puppet-configuration-management/` (Lesson 35: Puppet Configuration Management). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
- Keep runnable helpers under `scripts/`; keep sample source, manifests, policies, notes, and scenario files in clearly named subfolders.
...
```bash
cd 35-puppet-configuration-management
./scripts/validate.sh
```
...
- Use fenced code blocks with language hints such as `bash`, `yaml`, `json`, `ruby`, or `text`.
...
```bash
git diff --check
find 35-puppet-configuration-management -path '*/scripts/*.sh' -exec sh -n {} \
cd 35-puppet-configuration-management && ./scripts/validate.sh
```
