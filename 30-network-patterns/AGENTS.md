...
These instructions apply to `30-network-patterns/` (Lesson 30: Network Patterns). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 30-network-patterns -path '*/scripts/*.sh' -exec sh -n {} \;
cd 30-network-patterns && ./scripts/validate.sh
```
