...
These instructions apply to `19-redis-caching-load-balancing/` (Lesson 19: Redis Caching and Heavy Load Balancing for Distributed Systems). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 19-redis-caching-load-balancing -path '*/scripts/*.sh' -exec sh -n {} \;
cd 19-redis-caching-load-balancing && ./scripts/validate.sh
```
