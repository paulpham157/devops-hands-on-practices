...
These instructions apply to `00-docker-fundamentals/` (Lesson 00: Docker Fundamentals). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 00-docker-fundamentals -path '*/scripts/*.sh' -exec sh -n {} \;
