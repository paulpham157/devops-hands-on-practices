...
These instructions apply to `13-vaults-and-secrets-managers/` (Lesson 13: Vaults and Secrets Managers). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
cd 13-vaults-and-secrets-managers
...
```bash
git diff --check
find 13-vaults-and-secrets-managers -path '*/scripts/*.sh' -exec sh -n {} \;
```
