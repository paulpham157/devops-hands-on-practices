...
These instructions apply to `37-teamcity-ci-pipelines/` (Lesson 37: TeamCity CI Pipelines). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 37-teamcity-ci-pipelines -path '*/scripts/*.sh' -exec sh -n {} \
cd 37-teamcity-ci-pipelines && ./scripts/validate.sh
```
