...
These instructions apply to `02-kubernetes-nginx/` (Lesson 02: Kubernetes Nginx Deployment). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 02-kubernetes-nginx -path '*/scripts/*.sh' -exec sh -n {} \;
```
