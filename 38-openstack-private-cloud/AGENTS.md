...
These instructions apply to `38-openstack-private-cloud/` (Lesson 38: OpenStack Private Cloud). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 38-openstack-private-cloud -path '*/scripts/*.sh' -exec sh -n {} \
cd 38-openstack-private-cloud && ./scripts/validate.sh
```
