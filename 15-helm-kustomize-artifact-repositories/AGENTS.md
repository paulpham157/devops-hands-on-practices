...
These instructions apply to `15-helm-kustomize-artifact-repositories/` (Lesson 15: Helm, Kustomize, and Artifact Repositories). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 15-helm-kustomize-artifact-repositories -path '*/scripts/*.sh' -exec sh -n {} \;
