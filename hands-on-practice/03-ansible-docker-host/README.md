...
Use Ansible to configure the Vagrant Ubuntu VM from `lab-setup` as a Docker host.

The default inventory targets the VM from `lab-setup/settings.yaml`:
...
```bash
cd ../hands-on-practice/03-ansible-docker-host
```
