...
```bash
cd hands-on-practice/12-argocd-gitops
...
Argo CD must pull from a Git repository. Push this course repository to GitHub, GitLab, or another Git server first, then set `REPO_URL`.
...
APP_PATH=hands-on-practice/12-argocd-gitops/gitops-repo/apps/course-web/overlays/dev
```
