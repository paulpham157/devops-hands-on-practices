...
APP_PATH="${APP_PATH:-hands-on-practice/12-argocd-gitops/gitops-repo/apps/course-web/overlays/dev}"

if [ -z "$REPO_URL" ]; then
  echo "REPO_URL is required, for example:" >&2
  echo "REPO_URL=https://github.com/YOUR_ORG/docker-course.git ./scripts/render-application.sh" >&2
  exit 1
fi
