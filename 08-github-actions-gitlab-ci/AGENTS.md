...
These instructions apply to `08-github-actions-gitlab-ci/` (Lesson 08: GitHub Actions and GitLab CI). Follow the root `AGENTS.md` as the general policy, then use this file for lesson-specific work.
...
```bash
git diff --check
find 08-github-actions-gitlab-ci -path '*/scripts/*.sh' -exec sh -n {} \;
```
